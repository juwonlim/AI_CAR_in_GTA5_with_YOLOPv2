    ...
    for row in scan_rows:
        ...
        if len(indices) > 0:
            ...
            if debug_img is not None:
                ...
                cv2.line(debug_img, (0, row), (w, row), (0, 0, 255), 1)  # ← 여기에 포함시켜야 함,  차선 그려주는 코드이나 동작안함, debug_img가 차선이라는??
...
def draw_virtual_centerline(output_img, center_points):
    ...
    cv2.polylines(output_img, [pts], isClosed=False, color=(0, 255, 255), thickness=2) #이게 차선 그려주는 것
    return output_img
