...
def detect_realtime(model, device):
    ...
    while True:
        ...
        vis_img = cv2.resize(screen, (1280, 720))
         ...
         #여기서부터 yolopv2_model_demo_rev04.py파일 참조
        ...
        #img_tensor = torch.from_numpy(model_input).to(device) --> get_segmentation_masks와 중복인 3줄
        ...
        if running:
            ...
            if da_seg is None or ll_seg is None:
                ...
                continue
                 ...
                 # Lane Mask 시각화
            if ll_seg is None or ll_seg.shape[0] == 0:
            ...
            # 중심선 추출 및 시각화
            centerline, left_points, right_points = extract_centerline_from_mask(ll_seg, draw_img)
