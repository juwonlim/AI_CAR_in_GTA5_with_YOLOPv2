...
def check_front_vehicle_distance(pred, image_shape, safe_distance_px=120):
    ...
    for det in pred:
        ...
        if int(cls) != 2:
            continue
        if conf < 0.65:  # 기존보다 높임
            continue
        if abs(cx - mid_x) > 50: # > 50 일떄 무시 ---> 중심 50이내만 사용,  cx가 중심선에서 50픽셀 이상 벗어난 경우 → 무시 (continue) ,즉, 50픽셀 이내에 있는 것만 "앞차"로 간주
            continue
        if box_height < 50:  # 너무 작은 박스 제거 --> 이는 box_height >= 50 인 경우에만 통과시킨다는 의미
            continue

        if min_distance is None or box_height > min_distance:
            min_distance = box_height

    if min_distance and min_distance > safe_distance_px:
        return False  # 거리 충분 → 출발 가능
    return True  # 너무 가까움 → 감속 또는 정지
"""
