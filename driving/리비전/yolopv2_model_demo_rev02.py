...
#아래 내가 추가한 경로
weight_path = 'E:/gta5_project/AI_CAR_in_GTA5_with_YOLOPv2/yolopv2_inference/data/weights/yolopv2.pt'
...
def detect():
    ...
    # Load model
    stride =32
    #model  = torch.jit.load(weights) --> 이게 원본
...
def detect_realtime():
    ...
    model = torch.jit.load(weights[0]) #weight는 list임 , 그래서 이렇게!!
    ...
    while True:
        ...
        img = letterbox(im0, imgsz, stride=32)[0]
        ...
        for i, det in enumerate(pred):
            ...
            cv2.namedWindow("YOLOPv2_Result", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("YOLOPv2_Result", 640, 360)
            cv2.imshow("YOLOPv2_Result", im0)
            cv2.moveWindow("YOLOPv2_Result", 600, 750)
