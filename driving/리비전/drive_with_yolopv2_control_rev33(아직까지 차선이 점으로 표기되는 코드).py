...
def detect_realtime(model, device):
    ...
    while True:
        ...
        model_input = letterbox(vis_img_resize, new_shape=640)[0]
        ...
        vis_img_resize = cv2.resize(screen, (1280, 720)) #letterbox 후 vis_img의 원래 크기를 파악
        ...
        if running:
            ...
            # 차선 중심선 시각화
            centerline = extract_centerline_from_mask(ll_seg, draw_img)
            for (x, y) in centerline:
                cv2.circle(draw_img, (x, y), 2, (0, 255, 255), -1)
