...
def calculate_offset_from_lane_mask(mask, fallback_offset=0,debug_img=None):
    """
    YOLOPv2로 추출한 차선 마스크(mask)에서 중심선을 추정하고
    화면 중심으로부터의 offset(좌우 거리 차이)을 반환.
    
    - 좌우 차선이 모두 인식되면 중심선을 평균으로 계산
    - 한쪽 차선만 인식되면 일정 offset(±80)을 더해 중심선 추정 (아이디어1 적용)
    - 다수의 중심선 포인트를 기반으로 polyfit 보간(곡선 추세선) 적용
    - 마지막에 화면 중심 대비 offset 계산하여 반환
    """

    h, w = mask.shape  # 마스크 영상 크기: height, width

    num_rows_to_scan = 10  # 총 10줄만 샘플링 (퍼포먼스 최적)
    
    # 하단 100픽셀만 추출 (정확한 조향 위한 직선 추정)
    #scan_rows = np.linspace(h - 100, h - 1, num_rows_to_scan).astype(int)
    
    # [추천] 하단 1/3로 바꾸려면 아래 사용:
    #scan_rows = np.linspace(int(h * 2 / 3), h - 1, num_rows_to_scan).astype(int) # 스캔영역


     # [추천] 하단 1/2로 바꾸려면 아래 사용:
    #scan_rows = np.linspace(int(h * 1 / 2), h - 1, num_rows_to_scan).astype(int)
    scan_rows = np.linspace(int(h * 0.5), h - 1, num_rows_to_scan).astype(int)



    #debug_img가 None일 경우 cv2.line()에서 에러가 발생할 수 있으므로 조건문 보호 필요
    if debug_img is not None:
        for row in scan_rows:
            cv2.line(debug_img, (0, row), (w, row), (0, 0, 255), 1)

    
    #calculate_offset_from_lane_mask()에서 스캔 구간을 색으로 표시하는 코드
    for row in scan_rows:
        cv2.line(debug_img, (0, row), (w, row), (0, 0, 255), 1)

    # 왼쪽/오른쪽 차선의 평균 x좌표를 저장할 리스트
    left_xs, right_xs, ys = [], [], []

    for row in scan_rows:
        indices = np.where(mask[row] > 0)[0]  # row에서 차선이 있는 픽셀 위치들

        if len(indices) > 0:
            # 절반 기준으로 왼쪽/오른쪽 차선 분리
            left = indices[indices < w // 2]
            right = indices[indices >= w // 2]

            # 각 차선의 평균 위치 저장, 없으면 None
            left_xs.append(np.mean(left) if len(left) > 0 else None)
            right_xs.append(np.mean(right) if len(right) > 0 else None)
            ys.append(row)  # 해당 줄(y 위치) 저장

    # 중심선 추정 (좌우 평균 또는 평행 offset 기준)
    center_xs = [] #	좌/우 기반 가상 중심선
    last_known = None  # 이전의 중심선 기억 → 끊긴 부분 보완용

    for l, r in zip(left_xs, right_xs):
        if l is not None and r is not None:
            c = (l + r) / 2           # 양쪽 모두 있을 경우 평균 사용
            last_known = c
        elif l is not None:
            c = l + 80                # 왼쪽만 있을 경우 오른쪽으로 평행 offset
            last_known = c
        elif r is not None:
            c = r - 80                # 오른쪽만 있을 경우 왼쪽으로 평행 offset
            last_known = c
        else:
            c = last_known            # 둘 다 없으면 이전 중심 사용
        center_xs.append(c)

    # 유효한 중심 좌표 리스트 (x, y) 쌍으로 구성
    valid_centers = [(x, y) for x, y in zip(center_xs, ys) if x is not None]

    if len(valid_centers) < 3:
        print("[INFO] 차선 없음 → 정지")
        #return fallback_offset, False
        return fallback_offset, False, []  #  항상 3개 반환

    # 추세선 보간 (곡선을 다항식으로 근사)
    x_vals, y_vals = zip(*valid_centers)
    poly = np.polyfit(y_vals, x_vals, deg=2)            # 2차 다항식 근사 #polyfit, polyval -->곡선 보간 (조향 안정성 개선)
    center_x = np.polyval(poly, h - 1)                  # 화면 하단(y = h-1)에서 중심선의 x값 예측

    # 화면 중심 대비 offset 계산 (왼쪽이면 음수, 오른쪽이면 양수)
    offset = center_x - (w / 2) #	차량 화면 중심 대비 거리 차이 (조향에 사용)
    ...
    if all([l is not None and r is not None for l, r in zip(left_xs, right_xs)]):
        ...
        print("[INFO] 차선 인식 실패")
        #return fallback_offset, False
        return fallback_offset, False, []   #  세 개 반환 (offset, trigger, center_points)

    #return offset, True  # offset: 조향 제어용, True: 가속 가능 여부
    center_points = valid_centers
    return offset, True, center_points
...
def draw_virtual_centerline(output_img, center_points):
    ...
    Parameters:
    - output_img: 원본 이미지 (복사본이 전달됨)
    - center_points: (x, y) 형태의 중심선 좌표 리스트

    Returns:
    - output_img: 중심선이 그려진 이미지
    """

    ''' 
    #기존 코드
    # 중심 포인트가 2개 이상 있어야 선을 그림
    if len(center_points) >= 2:
        pts = np.array(center_points, dtype=np.int32)  # numpy 정수 배열로 변환
        cv2.polylines(output_img, [pts], isClosed=False, color=(0, 255, 255), thickness=2)  # 노란 중심선 그리기
    '''    
    ...
    cv2.polylines(output_img, [pts], isClosed=False, color=(0, 255, 255), thickness=2)
