...
def real_time_detect(screen, model, device, half, imgsz=640):
    """
    GTA5 실시간 영상 프레임(screen)을 받아 YOLOPv2로 추론하고 시각화까지 수행하는 함수.
    detect()에서 쓰던 일부 구조를 간소화하고, 외부 루프에서 model, device 재사용을 가능하게 설계.
    """

    # -------- 전처리 --------
    image_vis = cv2.resize(screen, (1280, 720))  # 원본 시각화용
    image = cv2.resize(image_vis, (imgsz, imgsz))  # 입력 사이즈 맞춤
    image = image[:, :, ::-1].transpose(2, 0, 1)  # BGR → RGB → CHW
    image = np.ascontiguousarray(image)

    img_tensor = torch.from_numpy(image).to(device)
    ...
    return {
        ...
        'drivable_area_mask': da_seg_mask,
