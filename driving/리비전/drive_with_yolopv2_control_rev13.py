...
def calculate_offset_from_lane_mask(mask, fallback_offset=0):
    ...
    # scan_rows = np.linspace(int(h * 2 / 3), h - 1, num_rows_to_scan).astype(int)
...
def draw_virtual_centerline(mask, output_img):
    ...
    left_xs, right_xs, ys = [], [], [] #	차선의 평균 위치 저장
    center_points = []
...
def main_drive_loop():
    ...
    while True:
        ...
        if controller.is_auto_drive_enabled():  # Y 키 눌러 자율주행 활성화된 경우
            ...
            output_img = draw_virtual_centerline(visual_img.copy(), center_points) #center_points 기반으로 가상 중앙선 그리기
