...
def calculate_offset_from_lane_mask(mask, fallback_offset=0):
    ...
    # 차선 인식 상태 파악
    left_found = any(l is not None for l in left_xs)
    right_found = any(r is not None for r in right_xs)
    ...
    if len(valid_centers) < 5:
        return fallback_offset, False  # offset, 촉발 안 됨

    center_xs, ys = zip(*valid_centers)
    poly = np.polyfit(ys, center_xs, deg=2)
    bottom_y = h - 1
    estimated_center_x = np.polyval(poly, bottom_y)
    offset = estimated_center_x - w / 2

    return offset, (left_found or right_found)
...
def draw_virtual_centerline(mask, output_img):
    ...
    valid_points = [(int(x), int(y)) for x, y in zip(center_xs, ys) if x is not None]
    
    # 시각화
    if len(valid_points) >= 5:
        points = np.array(valid_points, dtype=np.int32).reshape((-1, 1, 2))
        cv2.polylines(output_img, [points], isClosed=False, color=(0, 255, 255), thickness=2)

    # 디버깅용 인식 상태 출력
    if left_found and right_found:
        print("[INFO] 양쪽 차선 인식 → 가속")
    elif left_found:
        print("[INFO] 왼쪽 차선 인식 → 가속")
    elif right_found:
        print("[INFO] 오른쪽 차선 인식 → 가속")
    else:
        print("[INFO] 차선 인식 실패")

    return output_img, left_found or right_found
